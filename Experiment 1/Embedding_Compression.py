...
def float_to_int(arr, precision=2):
    """Converts a list of floats to integers based on the specified precision."""
    factor = 10 ** precision
...
def hamming_distance(s1, s2):
    ...
    if any(isinstance(item, float) for item in s1) or any(isinstance(item, float) for item in s2):
        s1_int = float_to_int(s1)
        s2_int = float_to_int(s2)

    # Calculate Hamming distance
    distance = sum(bin(byte1 ^ byte2).count('1') for byte1, byte2 in zip(s1_int, s2_int))
    return distance/len(s1_int)
...
for strategy in inputs:
    ...
    compress = []
    for byte_string in strategy:
        compressor.compress(bytes.fromhex(byte_string))
        compressed = compressor.flush(zlib.Z_FULL_FLUSH)
        compress.append(list(compressed))
    max_sequence_length = max(len(indices) for indices in compress)
    print(max_sequence_length)
    padded_sequences = pad_sequences(compress, maxlen=max_sequence_length, padding='post')
    ...
    if index == 0:
        tracker.start()
    seen_embeddings = pca.fit_transform(x_first_70)
    if index == 0:
        tracker.stop()
    unseen_embeddings = pca.transform(x_last_30)
    if index == 0:
    ...
    if index == 0:
        ...
        print(metrics.homogeneity_completeness_v_measure(labels_sample , AgglomerativeClustering(n_clusters=22).fit_predict(np.array(features_sample).astype(np.float16))))
    #similarity_distance(seen_embeddings, class_index_90, dataset[index])
