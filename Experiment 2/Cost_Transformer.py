...
@profile
def predict_embeddings(strategy, max_sequence_length, embedding_model):
    new_characters = [list(bytes.fromhex(byte_string)) for byte_string in strategy[0:10000]]
    long_sequences = pad_sequences(new_characters, maxlen=max_sequence_length, padding='post')
    final = embedding_model.predict(long_sequences)
    return final
...
embedding = predict_embeddings(strategy, max_sequence_length, embedding_model)
classify.fit(embedding, labels[0:10000]) 
tracker.start()
start_time = time.time()
final = predict_embeddings(strategy, max_sequence_length, embedding_model)
end_time = time.time()
